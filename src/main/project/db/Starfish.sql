...
/*      CONSTRAINT `FKR_1_ID` FOREIGN KEY (`codiceCliente`) REFERENCES `CLIENTI`(`codiceCliente`),
...
CREATE TABLE `MEMBRI` (
    ...
    /* CONSTRAINT `FKR_ID` FOREIGN KEY (`giorno`, `mese`) REFERENCES `TURNI_DI_LAVORO`(`giorno`, `mese`) */); 
...
CREATE TABLE `TIPOLOGIE` (
    ...
    CONSTRAINT `FK_TIPOLOGIE_LETTINI` FOREIGN KEY (`codiceLettino`) REFERENCES `LETTINI`(`codiceLettino`),
    CONSTRAINT `FK_TIPOLOGIE_PEDALO` FOREIGN KEY (`codicePedalò`) REFERENCES `PEDALO`(`codicePedalò`),
    CONSTRAINT `FK_TIPOLOGIE_TAVOLI` FOREIGN KEY (`numeroTavolo`) REFERENCES `TAVOLI`(`numeroTavolo`)
);
...
CREATE TABLE `TURNI_DI_LAVORO` (
     ...
     `mese` INT UNSIGNED NOT NULL CHECK (`mese` BETWEEN 1 AND 12),
     CONSTRAINT `ID_TURNI_DI_LAVORO_PK` PRIMARY KEY (`giorno`,`mese`));
